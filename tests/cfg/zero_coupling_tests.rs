...
mod tests {
    ...
    fn test_zero_coupling_json_workflow() -> Result<()> {
        ...
        register_auto_with_type::<WebServer, WebServerConfig>()?;
...
"#, type_name);
    ...
    fn test_multiple_service_types() -> Result<()> {
        // 注册多个不同的服务类型
        register_auto_with_type::<WebServer, WebServerConfig>()?;
        register_auto_with_type::<Database, DatabaseConfig>()?;
        register_auto_with_type::<Cache, CacheConfig>()?;
    ...
    fn test_manual_type_name_registration() -> Result<()> {
        // 使用自定义类型名注册
        register_auto::<WebServer, WebServerConfig>("custom_web_server")?;
