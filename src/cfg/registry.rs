...
/// 3. 创建透明的配置适配器
/// 4. 符合 Rust 惯用法
pub fn register<T, Config>(type_name: &str) -> Result<()>
where
    T: Send + Sync + 'static,
...
/// 带自动类型名称生成的智能注册
pub fn register_auto<T, Config>() -> Result<()>
where
    T: Send + Sync + 'static,
    Config: DeserializeOwned + Clone + Send + Sync + 'static,
    T: From<Config>,
{
    ...
    // 使用完整名称注册
    register::<T, Config>(&full_type_name)?;

    // 使用简短名称注册
    register::<T, Config>(&short_type_name)?;
...
mod tests {
    ...
    fn test_register_auto_with_type() -> Result<()> {
        register_auto::<TestService, TestConfig>()?;
    ...
    fn test_register_auto_manual_type_name() -> Result<()> {
        ...
        register::<CustomService, CustomConfig>("custom_service")?;
    ...
    fn test_duplicate_registration_with_different_types() -> Result<()> {
        // 注册第一个类型
        register::<TestService, TestConfig>("same_type_name")?;
        ...
        register::<AnotherService, AnotherConfig>("same_type_name")?;
    ...
    fn test_generate_short_type_name() {
        ...
        let full_name =
            "rustx::kv::store::map_store::MapStore<alloc::string::String, alloc::string::String>";
    ...
    fn test_trait_and_concrete_type_registry_independent() -> Result<()> {
        ...
        // 使用相同的名称注册到两个不同的注册表
        register::<ImplA, ImplAConfig>("same-name")?;
