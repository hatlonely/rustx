...
pub use aws_s3_object_store::{S3ObjectStore, S3ObjectStoreConfig};
pub use ali_oss_object_store::{AliyunOssObjectStore, AliyunOssObjectStoreConfig};
...
pub fn register_object_store() {
    // 注册 S3
    register_trait::<S3ObjectStore, dyn ObjectStore, S3ObjectStoreConfig>(
        "S3ObjectStore"
    ).expect("Failed to register S3ObjectStore");

    // 注册阿里云 OSS
    register_trait::<AliyunOssObjectStore, dyn ObjectStore, AliyunOssObjectStoreConfig>(
        "AliyunOssObjectStore"
    ).expect("Failed to register AliyunOssObjectStore");
